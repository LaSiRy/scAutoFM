...
def config_to_structure(config_dict):
    depth = 12  # 固定 depth
    
    lora_dim = config_dict['lora_dim']
    s_adapter_dim = config_dict['s_adapter_dim']
    p_adapter_dim = config_dict['p_adapter_dim']
    prefix_dim = config_dict['prefix_dim']
    
    lora_depth = config_dict['lora_depth']
    s_adapter_depth = config_dict['s_adapter_depth']
    p_adapter_depth = config_dict['p_adapter_depth']
    prefix_depth = config_dict['prefix_depth']
    
    structure = {
        "lora_dim": [lora_dim] * lora_depth + [0] * (depth - lora_depth),
        "s_adapter_dim": [s_adapter_dim] * s_adapter_depth + [0] * (depth - s_adapter_depth),
        "p_adapter_dim": [p_adapter_dim] * p_adapter_depth + [0] * (depth - p_adapter_depth),
        "prefix_dim": [prefix_dim] * prefix_depth + [0] * (depth - prefix_depth)
    }
...
class BayesSearch(object):
    ...
    def __init__(self, args, device, model, model_without_ddp, choices, val_loader, test_loader, output_dir, assessment, logger):
        ...
        self.search_space = [
            Categorical([5, 10, 50, 100], name='lora_dim'),
            Categorical([5, 10, 50, 100], name='s_adapter_dim'),
            Categorical([5, 10, 50, 100], name='p_adapter_dim'),
            Categorical([5, 10, 25, 50], name='prefix_dim'),
            Categorical([3, 6, 9, 12], name='lora_depth'),
            Categorical([3, 6, 9, 12], name='s_adapter_depth'),
            Categorical([3, 6, 9, 12], name='p_adapter_depth'),
            Categorical([3, 6, 9, 12], name='prefix_depth'),
        ]
    ...
    def is_legal(self, cand):
        ...
        cand = self.config_to_cand(config_dict)
    ...
    def search(self, patience=5):
        ...
        # 假设你已定义搜索空间
        space = self.search_space   # List of skopt dimensions

        # history: 存储所有评估的结构及其评估值
        self.history = []

        @use_named_args(space)
        def objective(**params):
            config_key = json.dumps(params, sort_keys=True)
            if config_key in self.vis_dict:
                return -self.vis_dict[config_key][self.assessment]
            
            sampled_config = config_to_structure(params)
            score = evaluate(
                self.val_loader, self.model, self.task_type, self.device,
                batch_size=int(2 * self.args.batch_size),
                amp=self.args.amp,
                mode='retrain',
                retrain_config=sampled_config
            )
            
            self.vis_dict[config_key] = {
        ...
        while self.epoch < self.max_epochs:
            ...
            result = gp_minimize(
                ...
                x0=[list(h[0].values()) for h in self.history] if self.history else None,
                y0=[-h[1] for h in self.history] if self.history else None,
                verbose=False
            )
            params = result.x  # list，当前推荐的超参数点
            score = -result.fun  # 因为 skopt 默认最小化，转成最大化分数

            # 打印本轮搜索结果
            print(f"Epoch {self.epoch} | Current params: {params} | Score: {score:.6f}")

            current_best_score = -result.fun  # 转回 maximization
        ...
        print("Search finished. Best config:")
        best_config = max(self.vis_dict.items(), key=lambda x: x[1][self.assessment]['acc'])
        print("Config:", best_config[0])
        print("Score:", best_config[1][self.assessment]['acc'])
...
def main(args):
    ...
    t = time.time()
    searcher = BayesSearch(args, device, model, model_without_ddp, choices, data_loader_val, data_loader_val, args.output_dir, args.assessment, logger)
