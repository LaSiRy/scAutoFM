...
ENSEMBL_DICTIONARY_FILE = '../Geneformer/geneformer/gene_name_id_dict_gc30M.pkl'
TOKEN_DICTIONARY_FILE = '../Geneformer/geneformer/token_dictionary_gc30M.pkl'
...
def read_dictionaries(
    ...
    if not file_found:
        print("error: No raw data for processing found within provided directory. \
...
def get_gene_list(dict_list, mode):
    ...
    if isinstance(dict_list, list):
        ...
        print(f"error: \
            ...
            {type(dict_list)} is not the correct format."
        )
        raise
...
def isp_aggregate_grouped_perturb(cos_sims_df, dict_list, genes_perturbed):
    ...
    if isinstance(genes_perturbed, list):
        if len(genes_perturbed) > 1:
            gene_ids_df = cos_sims_df.loc[
                np.isin(
                    ...
                    [set(gene) for gene in genes_perturbed],
                ),
                :,
            ]
        else:
        ...
        print(
            "error: aggregate_data is for perturbation of single gene or single group of genes. genes_to_perturb should be formatted as list."
        )
        raise

    if gene_ids_df.empty:
        print("error: genes_to_perturb not found in data.")
        raise
    ...
    for token, symbol in zip(tokens, symbols):
        cos_shift_data = []
        for dict_i in dict_list.values():
...
class InSilicoPerturberStats:
    ...
    def validate_options(self):
        for attr_name, valid_options in self.valid_option_dict.items():
            ...
            if not valid_type:
                print(
                    f"error: Invalid option for {attr_name}. \
                    Valid options for {attr_name}: {valid_options}"
                )
                raise
        ...
        if self.cell_states_to_model is not None:
            if len(self.cell_states_to_model.items()) == 1:
                ...
                if (
                    ...
                    print(
                        "error: Please specify 'state_key', 'start_state', and 'goal_state' in cell_states_to_model."
                    )
                    raise
                ...
                if (
                    ...
                    print("error: All states must be unique.")
                    raise

                if self.cell_states_to_model["alt_states"] is not None:
                    if not isinstance(self.cell_states_to_model["alt_states"], list):
                        print(
                            "error: self.cell_states_to_model['alt_states'] must be a list (even if it is one element)."
                        )
                        raise
