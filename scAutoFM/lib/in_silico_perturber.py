...
class InSilicoPerturber:
    valid_option_dict = {
        ...
        "model_type": {"GeneClassifier", "CellClassifier"},
        "num_classes": {int},
        ...
        *(TBA)* perturb_rank_shift : None, {1,2,3}
        ...
        if (self.cls_token_id is not None) and (self.eos_token_id is not None):
            ...
            if "cls" in self.emb_mode:
                print(f"error: \
                    emb_mode set to {self.emb_mode} but <cls> or <eos> token not in token dictionary."
                )
                raise
        ...
        if self.anchor_gene is None:
            ...
            try:
                ...
                print(f"error: Anchor gene {self.anchor_gene} not in token dictionary.")
                raise
        ...
        if self.genes_to_perturb == "all":
            ...
            if len(missing_genes) == len(self.genes_to_perturb):
                ...
                print(f"warning: \
                    Genes to perturb {missing_genes} are not in token dictionary."
                )
    ...
    def validate_options(self):
            ...
            for key, value in self.filter_data.items():
                if not isinstance(value, list):
                    ...
                    print(f"warning: \
                        ...
                        Changing {key} value to list ([{value}])."
                    )
        ...
        if self.cell_inds_to_perturb != "all":
            ...
            if (
                ...
                print("error: cell_inds_to_perturb must be positive.")
                raise

    def perturb_data(
        self, model, input_data_file, output_directory, output_prefix
    ):
        """
        ...
        if self.perturb_group is True:
            if (self.special_token) and ("cls" in self.emb_mode):
                self.isp_perturb_set_special(
                    model, filtered_input_data, output_path_prefix
                )
            else:
