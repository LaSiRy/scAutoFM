...
SEARCH_SPACE = {
    'LORA_DIM': [0, 5, 10, 25, 50],
    'S_ADAPTER_DIM': [0, 5, 10, 50, 100],
...
class ArchitectureEnv:
    def __init__(self, search_space):
        ...
        self.action_space = [len(search_space[k]) for k in self.keys]
        self.max_steps = len(self.keys)
        self.reset()
    ...
    def _get_state(self):
        return torch.tensor(self.state + [0] * (self.max_steps - len(self.state)), dtype=torch.float32)

    def step(self, action_idx):
        key = self.keys[self.step_idx]
        value = self.search_space[key][action_idx]
...
class PolicyNet(nn.Module):
    def __init__(self, search_space):
        super().__init__()
        self.input_dim = len(search_space)
        self.hidden = nn.Linear(self.input_dim, 64)
        self.action_heads = nn.ModuleList([
            nn.Linear(64, len(search_space[k])) for k in search_space
        ])

    def forward(self, state):
        x = F.relu(self.hidden(state))
        logits = [head(x) for head in self.action_heads]
        return logits  # 每步的动作 logits
        
class RLSearcher(object):
    ...
    def is_legal(self, cand):
        ...
        depth, lora_dim, adapter_dim,prefix_dim = decode_cand_tuple(cand)
        sampled_config = {}
    ...
    def evaluate_architecture(arch_tuple):
        # arch_tuple like: (0, 0, 0, 3, 3, 3) → (LORA_DIM, ..., ADAPTER_DEPTH)
        # 转为结构体 or 哈希 key
        cand = tuple(arch_tuple)

        if cand not in vis_dict:
            acc = your_train_and_eval(cand)  # 替换为实际训练评估
            vis_dict[cand] = {
                'acc': acc,
                'params': compute_params(cand),  # 如果你有
            }

        return vis_dict[cand]['acc']


    def search(search_space, max_episodes=100, patience=3):
        env = ArchitectureEnv(search_space)
        policy = PolicyNet(search_space)
        ...
        for episode in range(max_episodes):
            ...
            for step in range(env.max_steps):
                logits_list = policy(state)
                logits = logits_list[step]
            ...
            # 架构生成完毕
            arch = [search_space[k][i] for k, i in zip(search_space.keys(), actions_taken)]
            acc = evaluate_architecture(tuple(arch))  # 奖励

            # 更新策略网络（REINFORCE）
            loss = -torch.stack(log_probs).sum() * acc
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            # 更新 top_k 结果
            vis_dict[tuple(arch)] = {'acc': acc}
            top_k.append((tuple(arch), acc))
            top_k = sorted(top_k, key=lambda x: x[1], reverse=True)[:50]

            print(f"Episode {episode}: acc={acc}, best={top_k[0][1]}")
            ...
            if acc > best_score:
                ...
                if epochs_no_improve >= patience:
                    print(f"Early stopping at episode {episode}, best acc: {best_score}")
                    break

        print("Top-5 architectures:")
        for i, (arch, acc) in enumerate(top_k[:5]):
            print(f"{i+1}. {arch} -> acc: {acc:.4f}")
...
def main(args):
    ...
    t = time.time()
    searcher = RLSearcher(args, device, model, model_without_ddp, choices, data_loader_val, data_loader_val, args.output_dir, args.assessment, logger)
