...
class Attention(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        if self.LoRA_identity == False:
            q_w = self.query.weight + self.q_LoRA_b_weight @ self.q_LoRA_a_weight
            v_w = self.value.weight + self.v_LoRA_b_weight @ self.v_LoRA_a_weight
            q_scale = self.query_LoRA_m / (q_w.norm(p=2, dim=0) + 1e-9)
            v_scale = self.value_LoRA_m / (v_w.norm(p=2, dim=0) + 1e-9)
...
def _load_weights(model, basemodel, prefix: str = ''):
    ...
    for i, block in enumerate(model.blocks):
        ...
        if hasattr(block.attn, "query_LoRA_m"):
