...
def main(args):
    ...
    total_training_steps = int(len(data_loader_train) * args.epochs)
    ...
    if args.eval:
        ...
        print(f"Accuracy of the network on the valid dataset test {test_stats['acc']:.1f}")
        return
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            optimizer, lr_scheduler, args.task_type, args.nb_classes, device, epoch, loss_scaler, args.batch_size,
            args.clip_grad, amp=args.amp, fp16=args.fp16,
        ...
        if epoch % args.val_interval == 0 or epoch == args.epochs-1:
            test_stats = evaluate(data_loader_val, model, args.task_type, device, amp=args.amp, fp16=args.fp16, choices=choices, mode = args.mode, retrain_config=retrain_config,is_adapter=args.is_adapter,is_LoRA=args.is_LoRA,is_prefix=args.is_prefix)
            print(f"Accuracy of the network on the valid dataset test : {test_stats['acc']:.1f}")
            max_accuracy = max(max_accuracy, test_stats["acc"])
            print(f'Max accuracy: {max_accuracy:.2f}')
            if "auc" in test_stats:
                print(f"Auc of the network on the valid dataset test : {test_stats['auc']:.1f}")
                max_auc = max(max_auc, test_stats["auc"])
                print(f'Max auc: {max_auc:.2f}')

            if args.mode == 'retrain':
                print(test_stats)
                with open(f"{args.task_name}_results.pkl", "wb") as f:
                    import pickle
