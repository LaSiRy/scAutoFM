...
class EvolutionSearcher(object):
    ...
    def search(self, patience=3):
        ...
        best_score = None
        epochs_no_improve = 0
        self.all_candidates = set()  # 存储所有历史架构
        ...
        while self.epoch < self.max_epochs:
            ...
            # 保存当前轮候选架构到 all_candidates 中
            for cand in self.candidates:
                self.all_candidates.add(cand)

            self.memory.append([])
            for cand in self.candidates:
                self.memory[-1].append(cand)

            # 将历史所有候选架构合并排序，选 topK
            all_candidates_list = list(self.all_candidates)
            self.update_top_k(all_candidates_list, k=self.select_num, key=lambda x: self.vis_dict[x][self.assessment])
            self.update_top_k(all_candidates_list, k=50, key=lambda x: self.vis_dict[x][self.assessment])
            ...
            current_best_score = tmp_assessment[0]  
